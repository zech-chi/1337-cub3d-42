...
/*   Updated: 2024/07/15 18:55:50 by zech-chi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_mlx_image_to_window(t_cub *cub, mlx_image_t *img, int x, int y)
{
	...
	if (check == -1)
	{
		ft_putstr_fd("Error\nFailed to put image to window", 2, 1, RED);
		ft_free_data(cub);
		exit(1);
	}
}
...
mlx_image_t	*ft_mlx_new_image(t_cub *cub, uint32_t width, uint32_t height)
{
	...
	if (!image)
	{
		ft_putstr_fd("Error\nFailed to create image", 2, 1, RED);
		ft_free_data(cub);
		exit(1);
	}
	return (image);
